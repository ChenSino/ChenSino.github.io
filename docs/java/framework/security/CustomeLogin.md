...
~~~markdown
1. 我在很多教程中都看到他们有讲解自定义登录页面，但是我想说的是，都21世纪了，早都前后分离了，所以在Security配置中不要配置formLogin了，前后分离项目会直接在Controller自定义登录逻辑，一旦配置这个万一Security会自动生成表单登录那几个过滤器。这里重点强调一下，前后分离项目和前后不分离在Security中是大大的不一样！！大家学习的时候关键词检索要记得加上前后分离！！
2. 配置自定义过滤器，自定义过滤器用来处理请求，识别请求中是否有我们自定义的合法token,当然和不合法就要在过滤器的doFilter中自己写了。
~~~
